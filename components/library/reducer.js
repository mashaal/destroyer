...
export const libraryReducer = (state = {covers: [], tracks: [], albums: [], newest: false}, action) => {
  switch (action.type) {
    ...
    case 'ADD_ARTWORK': {
      ...
      let index = false
      albums.some((album, i) => {
        if ((album.root === action.album.root) && (album.artist === action.album.artist) && (album.title === action.album.title)) {
          index = i
          return
        }
      })
    ...
    case 'UPDATE_METADATA': {
      ...
      let albums = [...state.albums]
      let find = albums.filter(album => (album.title === action.track.album) && (album.artist === action.track.artist))
      if (find.length <= 0) {
        let cover = false
        if (state.covers.indexOf(path.join(action.track.root, 'cover.jpg')) >= 0) cover = path.join(action.track.root, 'cover.jpg')
        albums.push({artist: action.track.artist, title: action.track.album, time: action.track.time, root: action.track.root, cover: cover})
      }
